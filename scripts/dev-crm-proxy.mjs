...
async function handleCrmProxy(req, res) {
  ...
  if (!auth.authorized) { res.writeHead(401); res.end(JSON.stringify({ error: 'Unauthorized' })); return; }
...
async function executeAction(action, params) {
  switch (action) {
    ...
    case 'admin.update': {
      if (!hasPerm(params._auth, 'manage_admins')) throw new Error('Forbidden');
      const { email, displayName, permissions } = params;
      const updates = {};
...
server.listen(PORT, () => {
  console.log(`CRM proxy running on http://localhost:${PORT}`);
  if (!process.env.VITE_FIREBASE_API_KEY) console.log('Using default Firebase API key from code.');
});
