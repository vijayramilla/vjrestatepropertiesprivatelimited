...
import { MapPin, MessageCircle, Mail, Clock, ChevronDown, CheckCircle } from 'lucide-react';
            ...
            <div className="mt-[34px] space-y-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-black text-[16px] leading-relaxed">Koramangala 7th Block<br />Bangalore, Karnataka 560095</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <a href="https://wa.me/918088905957" target="_blank" rel="noopener noreferrer" className="hoverable text-black text-[16px] hover:text-gray-600 transition-colors">+91 8088905957</a>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <Mail size={20} className="text-black mt-1 flex-shrink-0" />
                <div>
                  <p className="text-gray-400 text-[11px] uppercase tracking-[0.15em] mb-1">Email</p>
                  <a href="mailto:info@vjrestate.com" className="hoverable text-black text-[16px] hover:text-gray-600 transition-colors">info@vjrestate.com</a>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <Clock size={20} className="text-black mt-1 flex-shrink-0" />
                <div>
                  <p className="text-gray-400 text-[11px] uppercase tracking-[0.15em] mb-1">Hours</p>
                  <p className="text-black text-[16px]">Mon – Sat, 10am – 7pm</p>
                </div>
              </div>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="lg:w-[38.2%]">
            {!submitted ? (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button type="submit" className="hoverable w-full py-4 bg-black text-white text-[13px] uppercase tracking-[0.1em] transition-colors hover:bg-gray-900">Submit</button>
              </form>
            ) : (
      ...
      <section className="bg-black py-[55px]">
        <div className="max-w-7xl mx-auto px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <p className="text-white text-[18px] mb-6">Looking to sell your property? We connect verified sellers with serious buyers.</p>
            <Link to="/submit-requirement" className="hoverable inline-flex items-center justify-center px-8 py-3 border border-white text-white text-[13px] uppercase tracking-[0.1em] transition-colors hover:bg-white hover:text-black">Submit Your Property</Link>
          </motion.div>
        </div>
      </section>
    </div>
