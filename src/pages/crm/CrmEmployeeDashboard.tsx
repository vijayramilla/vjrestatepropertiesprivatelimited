...
import {
  Clock, CheckCircle, TrendingUp, Eye,
...
import KycOnboarding from '@/components/crm/KycOnboarding';
...
export default function CrmEmployeeDashboard() {
  ...
  // KYC onboarding state (own dashboard only — admins review from the profile).
  const [kyc, setKyc] = useState<any>(null);
  const [kycOpen, setKycOpen] = useState(false);
  const [kycNudgeDismissed, setKycNudgeDismissed] = useState(false);
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      const [verify, meRes, statsRes, clientsRes, eventsRes, visitsRes, payrollRes, kycRes] = await Promise.all([verifyP, meP, statsP, clientsP, eventsP, visitsP, payrollP, kycP]);
      setIsEmployee(verify?.role === 'employee');
      setMe(meRes.data);
      setStats(statsRes.data);
      setClients(clientsRes.data?.clients ?? []);
      setClientsLocked(!preview && (clientsRes.data as any)?.locked === true);
  ...
  // KYC gate — shown as a popup nudge + an on-demand onboarding sheet, never a
  // big inline form. kycGet always returns an object; the row is null until the
  // employee starts, which still counts as "not started".
  const kycStatus = kyc?.kyc?.status ?? 'not_started';
  const kycRequired = me?.kyc_required !== false;
  const kycActionable = kycRequired && (kycStatus === 'not_started' || kycStatus === 'changes_requested');
  const closeKyc = () => setKycOpen(false);

  // Deep links like ?profile=1 (used by the shared portal top bar) open the
  // profile modal as soon as the employee data has loaded.
  useEffect(() => {
    if (preview || !me || searchParams.get('profile') !== '1') return;
    setProfileOpen(true);
    searchParams.delete('profile');
    setSearchParams(searchParams, { replace: true });
  }, [preview, me, searchParams, setSearchParams]);

  // Deep links like ?kyc=1 open the KYC sheet (locked My Clients gates use this).
  useEffect(() => {
    if (preview || !kyc || searchParams.get('kyc') !== '1') return;
    setKycOpen(true);
    searchParams.delete('kyc');
    setSearchParams(searchParams, { replace: true });
  }, [preview, kyc, searchParams, setSearchParams]);

  // If the admin approves while the sheet is open, close it automatically.
  useEffect(() => {
    if (kyc?.kyc?.status === 'verified') setKycOpen(false);
  }, [kyc]);
  ...
  const statCards = [
    {
      label: 'Work start', value: fmtTime12(stats?.work_start_time), sub: 'Scheduled shift start',
      icon: <Hourglass className="h-4 w-4" strokeWidth={1.8} />, cls: '', toneText: '',
    },
    {
      label: 'Shift ends', value: fmtTime12(stats?.auto_logout_time),
      sub: todayAtt?.check_in ? (todayAtt?.check_out ? 'Shift complete' : autoLogoutCountdown !== null && autoLogoutCountdown === 0 ? 'Auto-logout reached' : `in ${autoLogoutCountdown !== null ? Math.ceil(autoLogoutCountdown / 60) : '—'}m`) : 'Not clocked in yet',
      icon: <Clock className="h-4 w-4" strokeWidth={1.8} />, cls: '', toneText: '',
    },
    {
      label: 'Attendance',
      value: todayAtt?.check_in && !todayAtt?.check_out ? '✓ Present' : todayAtt?.check_out ? 'Completed' : 'Not marked',
      sub: todayAtt?.check_in ? (todayAtt?.check_out ? `${fmtTime12(todayAtt.check_in)} → ${fmtTime12(todayAtt.check_out)}` : `Since ${fmtTime12(todayAtt.check_in)}`) : 'Tap Clock In to start your day',
      icon: <BadgeCheck className="h-4 w-4" strokeWidth={1.8} />,
      cls: todayAtt?.check_in && !todayAtt?.check_out ? 'bg-emerald-50/80 text-emerald-700' : '',
      toneText: todayAtt?.check_in && !todayAtt?.check_out ? 'text-emerald-700' : '',
    },
    {
      label: 'This month', value: `${stats?.login_count ?? 0} logins`,
      sub: stats?.attendance_days ? `${stats.attendance_days} attendance days` : undefined,
      icon: <TrendingUp className="h-4 w-4" strokeWidth={1.8} />, cls: '', toneText: '',
    },
  ];

  // Compact day-snapshot strip shown inside the clock card (one source of truth,
  // no duplicate check-in/check-out blocks anywhere else on the page).
  const clockFooter = (
    <>
      {statCards.map((s) => (
        <div key={s.label} className={`min-w-0 bg-white px-3 py-2.5 sm:px-4 ${s.cls}`}>
          <p className="flex items-center gap-1.5 text-[8.5px] font-bold uppercase tracking-[0.14em] text-[#9ca3af]">
            {s.icon}
            <span className="truncate">{s.label}</span>
          </p>
          <p className={`mt-0.5 truncate font-['Inter',sans-serif] text-[13.5px] font-extrabold tracking-tight text-[#0A1628] tabular-nums ${s.toneText}`}>
            {s.value}
          </p>
          {s.sub && <p className="truncate text-[9.5px] font-semibold text-[#9ca3af]">{s.sub}</p>}
        </div>
      ))}
    </>
  );
  ...
  return (
    <EmployeePortalLayout
      tab="dashboard"
      me={preview ? null : me}
      notesAction={preview ? undefined : () => setNotesOpen(true)}
      onProfile={() => setProfileOpen(true)}
    >
      {preview && (
        <div className="mb-4 flex flex-wrap items-center gap-2 rounded-2xl border border-[#C9A84C]/40 bg-[#C9A84C]/[0.08] px-3 py-2.5 sm:px-4 sm:py-3">
      ...
      <div className="relative mb-4 overflow-hidden rounded-2xl border border-black/[0.06] bg-[#0A1628] shadow-[0_12px_40px_rgba(10,22,40,0.25)] sm:mb-6 sm:rounded-3xl">
        ...
        <div className="relative flex flex-col gap-4 p-4 sm:gap-5 sm:p-6 lg:flex-row lg:items-center lg:justify-between lg:p-7">
              ...
              <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                ...
                {faceToday && (
                  <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-emerald-500/15 px-2 py-0.5 text-[8.5px] font-bold uppercase tracking-wide text-emerald-300 ring-1 ring-emerald-400/40 sm:text-[9.5px]">
                    <ShieldCheck className="h-3 w-3" strokeWidth={2} /> <span className="hidden sm:inline">Face verified</span> ✓
                  </span>
                )}
              </div>
          ...
          {/* Greeting — hidden on small phones, desktop right side */}
          <div className="hidden w-full shrink-0 flex-row items-center justify-end gap-3 md:flex lg:w-auto">
            <p className="text-right text-[12px] font-semibold leading-snug text-white/60">
              {new Date(now).getHours() < 12 ? 'Good morning' : new Date(now).getHours() < 17 ? 'Good afternoon' : 'Good evening'},
              <span className="block text-white/85">{me.name?.split(' ')[0] || 'there'}</span>
            </p>
      ...
      {profileOpen && (
        <EmployeeProfileModal me={me} stats={stats} payroll={payroll} onClose={() => setProfileOpen(false)} onChanged={fetchAll} />
      )}
      ...
      {/* ═══════ KYC — floating popup nudge (clicks open the onboarding sheet) ═══════ */}
      {!preview && kyc && kycActionable && !kycNudgeDismissed && !kycOpen && (
        <div
          className="fixed inset-x-3 bottom-[calc(env(safe-area-inset-bottom)+5.25rem)] z-[75] mx-auto max-w-md rounded-2xl border border-[#C9A84C]/30 bg-white p-4 shadow-[0_24px_80px_rgba(10,22,40,0.35)] sm:inset-x-auto sm:bottom-6 sm:right-6 sm:max-w-sm"
          role="dialog"
          aria-label="KYC onboarding reminder"
        >
          <div className="flex items-start gap-3">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#C9A84C]/[0.14] text-[#96782A] ring-1 ring-[#C9A84C]/30">
              <ShieldCheck className="h-5 w-5" strokeWidth={1.8} />
            </span>
            <div className="min-w-0 flex-1">
              <p className="text-[13px] font-bold leading-snug text-[#0A1628]">
                {kycStatus === 'changes_requested' ? 'KYC needs a small correction' : 'Complete your KYC onboarding'}
              </p>
              <p className="mt-0.5 text-[11px] leading-relaxed text-[#6b7280]">
                {kycStatus === 'changes_requested'
                  ? 'The admin returned some documents. Review and resubmit to unlock your client workspace.'
                  : 'Verify your identity (Aadhaar & PAN) to unlock your client workspace — it takes under two minutes.'}
              </p>
            </div>
            <button
              onClick={() => setKycNudgeDismissed(true)}
              aria-label="Dismiss reminder"
              className="-mr-1 -mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-[#9ca3af] transition-colors hover:bg-black/[0.04] hover:text-[#0A1628]"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          <div className="mt-3 flex items-center gap-2">
            <CrmBtn
              variant="gold"
              className="min-h-[42px] flex-1 text-[12px]"
              onClick={() => { setKycNudgeDismissed(true); setKycOpen(true); }}
            >
              <ShieldCheck className="h-3.5 w-3.5" />
              {kycStatus === 'changes_requested' ? 'Review & resubmit' : 'Start onboarding'}
            </CrmBtn>
            <button onClick={() => setKycNudgeDismissed(true)} className="min-h-[42px] rounded-xl px-3 text-[11px] font-bold text-[#9ca3af] transition-colors hover:text-[#0A1628]">
              Later
            </button>
          </div>
        </div>
      ...
      {/* ═══════ KYC ONBOARDING SHEET — full flow, opened on demand ═══════ */}
      {!preview && kycOpen && kyc && (
        <div className="fixed inset-0 z-[90] flex items-end justify-center bg-[#050b14]/75 backdrop-blur-sm sm:items-center sm:p-4" onClick={closeKyc}>
          <div
            onClick={(e) => e.stopPropagation()}
            className="relative flex max-h-[92dvh] w-full max-w-2xl flex-col overflow-hidden rounded-t-[26px] bg-white shadow-[0_40px_120px_rgba(0,0,0,0.55)] sm:rounded-[26px]"
          >
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
              <KycOnboarding kyc={kyc} onChanged={fetchAll} onClose={closeKyc} />
            </div>
          </div>
        </div>
      )}

      {/* ═══════ DAY AT A GLANCE — one compact clock card with the day snapshot strip inside ═══════ */}
      {!preview && (
        <ClockInOut today={todayAtt} activeBreak={activeBreak} onChanged={fetchAll} footer={clockFooter} />
      )}

      {/* ═══════ OFFICE UPDATES & EVENTS — only shown when the admin has posted ═══════ */}
      {events.length > 0 && (
        <MotionReveal delay={0}>
          <CrmCard className="mb-4 mt-4 overflow-hidden sm:mb-6 sm:mt-6">
      ...
      {/* ═══════ MY CLIENTS — the main working grid (unlocked only after KYC when required) ═══════ */}
      {clientsLocked ? (
        <KycClientsGate kycStatus={kycStatus} onChanged={fetchAll} onOpen={() => setKycOpen(true)} />
      ) : (
