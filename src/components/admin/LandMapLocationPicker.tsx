  ...
  const handleApplyLink = async () => {
    ...
    try {
      ...
      if (backendResponse.success) {
        const { lat, lng, areaName, city, state, pincode, fullAddress } = backendResponse;
      ...
      // Map backend error stages to user-friendly messages
      const errorStage = backendResponse.errorStage;
      const errorMessages: Record<string, string> = {
        'short-url-resolution': "This short link couldn't be resolved — it may be expired or invalid. Try pasting the full Google Maps URL instead.",
        'url-parsing': "No location information found in this link. Paste a Google Maps URL, full address, or coordinates (e.g., 12.9716, 77.5946).",
        'geocoding-no-results': "Address not found. Try a different location, area name, or full address.",
        'backend-exception': "Server error. Please try again in a moment.",
        'input': 'Please paste a valid Google Maps link or address.',
        'http': 'Invalid request. Please try again.',
        'none': 'Could not process this location.'
      };
      ...
      try {
        ...
        setLinkError(fallbackErr instanceof Error ? fallbackErr.message : 'Failed to parse link');
      }
    } finally {
      setSearchLoading(false);
    }
