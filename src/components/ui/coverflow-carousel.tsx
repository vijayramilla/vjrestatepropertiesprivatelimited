...
export interface CoverflowSlide {
  ...
  /** Shows a shortlist (cart) button on the card; report clicks via onToggleSave. */
  savable?: boolean;
  /** Whether this slide is currently saved/shortlisted. */
  saved?: boolean;
  /** Fires when the save button on slide `index` is clicked. */
...
export interface CoverflowCarouselProps {
  ...
  /** Fires whenever the centred slide changes (drag, arrows, keyboard). */
            ...
            {slides.map((slide, index) => (
              ...
              >
                ...
                {slide.savable && (
                  <button
                    type="button"
                    aria-label={slide.saved ? 'Remove from shortlist' : 'Save property'}
                    onPointerDown={(event) => event.stopPropagation()}
                    onClick={(event) => {
                      event.stopPropagation();
                      slide.onToggleSave?.(index);
                    }}
                    className="absolute right-2.5 top-2.5 z-[150] flex h-8 w-8 items-center justify-center rounded-full bg-white/95 shadow-md backdrop-blur-sm transition-all hover:bg-white active:scale-90"
                  >
                    <BookmarkSimple
                      size={14}
                      weight={slide.saved ? 'fill' : 'regular'}
                      className={slide.saved ? 'text-[#C9A84C]' : 'text-gray-500'}
                    />
                  </button>
                )}
              </div>
